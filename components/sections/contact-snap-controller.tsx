...
function contactSections() {
  return Array.from(document.querySelectorAll<HTMLElement>(".contact-mobile-snap .contact-snap-section"));
}

export function ContactSnapController() {
  useEffect(() => {
    ...
    const clearAnimation = () => {
      ...
      document.documentElement.removeAttribute("data-ag-contact-snap-animating");
    };
    ...
    const animateScrollTo = (targetSection: HTMLElement) => {
      ...
      document.documentElement.setAttribute("data-ag-contact-snap-animating", "true");
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (!target?.classList.contains("contact-snap-section")) return;
    ...
    const settleToNearestSection = () => {
      ...
      if (currentSection.classList.contains("contact-faq-section") && currentSection.getBoundingClientRect().top < expectedTop - SNAP_SETTLE_TOLERANCE_PX) return;
